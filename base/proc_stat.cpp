...
{
	return strcmp(static_cast<const base::sys_mem_entry*>(a)->name,static_cast<const base::sys_mem_entry*>(b)->name);
}
...
bool base::get_system_stat(system_stat& ss,int flag)
{
	...
	if(flag & PROC_MEM){
		...
		for(beg=buf;beg<end;beg=lf+1){
			...
			*colon++ = '\0';
			...
			if(!lf) break;
			key.name = beg;
			sme = (sys_mem_entry*)bsearch(&key,mem_table,NUM_ELEMENTS(mem_table),sizeof(sys_mem_entry),
										  compare_sys_mem_entry);
...
bool base::get_system_usage(float& cpu,float& mem)
{
...
bool base::get_process_stat(pid_t id,process_stat& ps)
{
...
bool base::get_process_usage(pid_t id,float& cpu,float& mem,unsigned long long* uptime/*=NULL*/)
{
	system_stat ss;
	...
	unsigned long long pct = ps.utime+ps.stime, upt;
	upt = (unsigned long long)ss.ut.uptime - ps.starttime/hz;
	cpu = upt ? (100.0*pct/hz)/upt : 0.0;

	long page_size = sysconf(_SC_PAGESIZE) >> 10;
	mem = 100.0*ps.rss*page_size/ss.mi.main_total;
	
	if(uptime) *uptime = upt;
